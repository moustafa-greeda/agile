...
<?php
		// if(isset($_SESSION['last_project']))
		// 	echo "<h1>Latest Project => " . $_SESSION['last_project']."</br>";
	?>
...
<section class="table-project">
  ...
  <?php
      ...
      foreach($fetchData as $data){
        $query = "SELECT id_sprint,name_sprint FROM sprint WHERE id_project=".$data['id_project']."";
        $result = $conn->query($query);
          if ($result->num_rows > 0) {
            ...
            foreach($sprints as $sprint){
              ...
              $query = "SELECT id_user_stories,name_user_story FROM user_story WHERE id_sprint=".$sprint['id_sprint']."";
              $result = $conn->query($query);
              if ($result->num_rows > 0) {
